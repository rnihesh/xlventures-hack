...
A beat-by-beat runbook for the recorded demo. Run the live stack with a real `OPENAI_API_KEY` set (the engine always uses the real model when keyed) and a seeded demo workspace. The right column ties each beat to the judging rubric.

| Time | On screen | Say this | Rubric criterion |
| --- | --- | --- | --- |
| 0:00 | Inbox at `http://localhost:3200`, accounts ranked by risk. | "Operators drown in dashboards. This is a decision inbox: accounts ranked by churn risk and signal recency, not another alert feed. Northwind Logistics is critical, health 42, usage down 38 percent." | Use case: real pain, operator-ready. |
...
| 4:20 | Eval node on the "System overview" diagram. | "Why an eval harness: claims need proof. A golden-scenario harness scores five offline, deterministic suites (citation grounding, retrieval faithfulness, action match, trajectory validity, and org-scoped outcome lift), so every change is measured. Plus offline-deterministic fallbacks, shared contracts, and Alembic migrations make the whole thing reproducible on any laptop." |
