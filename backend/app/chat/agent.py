...
_SYSTEM_PROMPT = (
    ...
    "surfaces. You can also ACT: create_account and update_account to add or edit "
    "an account (notes become evidence), add_contact and list_contacts to manage "
    "contacts, list_runs to show recent runs, get_timeline for an account's audit "
    "trail, get_integrations for connector status, record_outcome to log a run's "
    "result into the learning loop, and web_search for external context. When the "
    "user asks to add, create, update, or manage an account or contact, USE the "
    "matching tool rather than saying you cannot. Use the provided tools to answer with real data instead of "
